...
/// 纯文本词典纠错器 (会议模式专用)
///
/// 与基座 `CorrectionDictionary` 分工不同:
/// - `CorrectionDictionary` 把词典正确词当 hint **喂给 ASR**(转写前, 输入端)。
/// - `DictionaryCorrector` 对 ASR 产出的**纯文本做强制替换纠错**(转写后, 输出端)。
///   会议转写只给纯文本, 拿不到 word-level 的 confidence/alternatives, 所以这里做的是
///   **整段文本上的确定性强制替换**——词典里的词本来就该无条件替换, 不依赖置信度。
///
/// 词典源: `~/.we/dictionary.json`(经 `WEDataDir.contextualDictURL`)。
/// 格式: `{"错词": "正确词", ...}`(错→对映射)。带热更新(改词典不用重启)。
///
/// 匹配规则 (防误伤):
...
final class DictionaryCorrector {
    ...
    private init() {
        reload(with: Self.loadDictionary())
        startWatching()
    }

    // MARK: - 词典加载 (接基座 WEDataDir, 不依赖 WEConfig)

    /// 从 ~/.we/dictionary.json 读 错→对 映射。文件不存在/格式非法时返回空。
    private static func loadDictionary() -> [String: String] {
        let url = WEDataDir.contextualDictURL
    ...
    /// 监听 dictionary.json 变更, 变了就重建规则 (热更新)。
    private func startWatching() {
        let path = WEDataDir.contextualDictURL.path
        // 文件不存在时不监听 (首次写入后需重启才生效; 词典一般已存在)。
        let fd = open(path, O_EVTONLY)
        guard fd >= 0 else { return }
        let src = DispatchSource.makeFileSystemObjectSource(
            fileDescriptor: fd,
            eventMask: [.write, .rename, .delete, .extend],
            queue: DispatchQueue(label: "we.dictionary-corrector-watch", qos: .utility)
        )
        src.setEventHandler { [weak self] in
            Task { @MainActor in self?.reload(with: Self.loadDictionary()) }
        }
